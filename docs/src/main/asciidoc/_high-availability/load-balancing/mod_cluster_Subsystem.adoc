...
For more information about the project visit the https://www.modcluster.io/[project website] and the https://docs.modcluster.io/[documentation website].
...
on server startup (e.g. via `-Djboss.node.name=myroute`).
...
To configure `instance-id` statically, configure the corresponding property in the Undertow subsystem:
...
By default, mod_cluster is configured for multicast-based discovery.
...
* ones that affects the configuration and require a restart of the subsystem
...
There are 3 operation that could be used to manipulate the list of reverse proxies the node is connected to.

[[list-proxies]]
==== list-proxies

Displays the reverse proxies that are connected to the node.
The proxies are either discovered by the advertise mechanism or by configured `proxies` attribute.
...
Removes a proxy from the discovered proxies or temporarily from the static proxies list.
...
Adds a proxy to the discovered proxies or temporarily to the proxy-list  attribute.
...
These operations allow to send context related commands to the reverse proxies.
These are also issued automatically when deploying or undeploying webapps.

[[enable-context]]
==== enable-context

Informs reverse proxies that the web context is ready receive requests.
...
Informs reverse proxies that it should not send any new session requests to the specifed web context.
...
Informs reverse proxies that it should not send requests to the web context.
...
[source,options="nowrap"]
----
[standalone@localhost:9990 /] /subsystem=modcluster/proxy=default/load-provider=dynamic/load-metric=cpu:remove
{
    "outcome" => "success",
    "response-headers" => {
        "operation-requires-reload" => true,
