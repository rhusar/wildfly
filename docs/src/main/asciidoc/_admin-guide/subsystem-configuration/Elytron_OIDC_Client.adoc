...
NOTE: If both the deployment and subsystem configuration are present, then the subsystem configuration will override the deployment values.
...
OpenID Connect Logout provides a single-sign-out feature that builds on the OpenID Connect authentication protocol.
A user can initiate a logout and have all sessions terminated without having to actively log out from every client.

A logout request whether through browser interaction or application-to-application
communication initiates the process by contacting the OpenId Provider, who communicates with all relevant parties to terminate active sessions.
There are different logout mechanisms for performing logout through the browser and through an application.  Front-Channel Logout is the browser based mechanism.
Back-Channel Logout is the application-to-application based mechanism.
Both Front-Channel and Back-Channel Logout are built on top of RP-Initiated Logout; it redirects the user's logout request to the OpenId Provider's logout endpoint, which then handles the logout process.

The configuration for this feature can be specified in the application's
`oidc.json` file or in the `secure-deployment` resource of the `elytron-oidc-client` subsystem.
...
----
{
  ...
  "provider-url" : "http://localhost:8180/auth/realms/demo",
  "ssl-required" : "external",
  "logout-path" : "portal/logout",
  "logout-callback-path" :
   "http://localhost:8080/customer-portal/goodbye-page",
  "back-channel-logout-session-invalidation-limit" : 25
   "credentials" : {
      "secret" : "234234-234234-234234"
   }
}
...
* `logout-path`: The path component of a URL.  It is uses to identify the URL that is used to start an OIDC logout request.
* `logout-callback-path`: An absolute URI.  It will be called after logout has been executed.  It is used by Front-channel and Back-channel logout.  The URI must be registered with the OpenId Provider.
* `post-logout-redirect-uri` : An absolute URI.  It will be called after logout has been executed.  It is used by RP-Initiated logout only.
 The URI must be registered with the OpenId Provider.
* `logout-session-required` :  When set to `true` the `sid` and `iss` parameters will be enforced when processing front-channel logout requests and the `sid` will be enforced when processing back-channel requests.  If set to `false` these parameters will not be checked and any authenticated request will invalidate the local user session.
...
<subsystem xmlns="urn:wildfly:elytron-oidc-client:1.0">
    <secure-deployment name="customer-portal.war">
        <provider>keycloak</provider>
        <client-id>customer-portal</client-id>
        <logout-path>portal/logout</logout-path>
        <logout-callback-path>http://localhost:8080/customer-portal/goodbye-page</logout-callback-path>
        <back-channel-logout-session-invalidation-limit>25</back-channel-logout-session-invalidation-limit>
    </secure-deployment>
</subsystem>
----


NOTE: This is a preview level feature; the WildFly server's stability level must be set to `preview`
or lower to enable it. The stability level must not be changed to `community` or `default` after configuring the
`logout` attributes. For more information about the stability levels of the WildFly server, please refer to the
link:Admin_Guide{outfilesuffix}[Admin Guide], particularly the
link:Admin_Guide{outfilesuffix}#Feature_stability_levels[Feature stability levels] section.
