...
public abstract class SessionExpirationTestCase extends AbstractClusteringTestCase {
    ...
    public void test(@ArquillianResource(SessionOperationServlet.class) @OperateOnDeployment(DEPLOYMENT_1) URL baseURL1,
        ...
        try (CloseableHttpClient client = TestHttpClientUtils.promiscuousCookieHttpClient()) {
            ...
            // Retry a couple of times since expiration in the remote case expiration depends on timing of hte reaper thread
            for (int attempt = 0; attempt < maxAttempts && !destroyed; attempt++) {
                    ...
                    if (!destroyed) {
                        ...
                        try {
                            ...
                            if (destroyed) {
                                ...
                                log.infof("Session destroyed within %d attempts/seconds.", attempt);
                            }
                        } finally {
                            HttpClientUtils.closeQuietly(response);
                        }
            ...
            Assert.assertTrue("Session has not been destroyed following expiration within " + maxAttempts + " seconds.", destroyed);
        }
    }
}
